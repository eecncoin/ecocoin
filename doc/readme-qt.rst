EcoCoin-qt: Qt4 GUI for EcoCoin
===============================
...
Alternatively, install Qt Creator and open the `EcoCoin-qt.pro` file.

An executable named `EcoCoin-qt` will be built.
...
A warning for people using the *static binary* version of EcoCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of EcoCoin is linked against libdb5.3.
...
installed causes EcoCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
